...
class VentaCreateDto {
    constructor({ empleadoId, detalles }) {
        this.empleadoId = typeof empleadoId === 'number' ? empleadoId : parseInt(empleadoId, 10);
        this.detalles = Array.isArray(detalles) 
            ? detalles.map(d => ({
                productId: typeof d.productId === 'number' ? d.productId : parseInt(d.productId, 10),
                priceId: typeof d.priceId === 'number' ? d.priceId : parseInt(d.priceId, 10),
                cantidad: typeof d.cantidad === 'number' ? d.cantidad : parseInt(d.cantidad, 10)
              }))
            : null;
    }
    ...
    validate() {
        ...
        if (!this.empleadoId || isNaN(this.empleadoId)) {
        ...
        // 2. Validar la lista de Detalles
        if (!this.detalles) {
            ...
            this.detalles.forEach((d, idx) => {
                ...
                if (!d.productId || isNaN(d.productId)) {
                ...
                if (d.cantidad === undefined || d.cantidad === null || isNaN(d.cantidad)) {
